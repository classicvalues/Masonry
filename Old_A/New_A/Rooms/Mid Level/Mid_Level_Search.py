...
output_directory = r"C:\Masonry\New_A\Rooms\Mid Level"
...
# Filter conditions
length_condition = (df['Length'] >= 36) & (df['Length'] <= 96)
width_condition = (df['Width'] >= 36) & (df['Width'] <= 96)
...
# Combine conditions for attachment containing both 'D' and 'T'
attachment_condition = df['Attachment'].str.contains('D', na=False) & df['Attachment'].str.contains('T', na=False)

# Ensure that attachment doesn't have T without D or D without T
valid_attachment_condition = df['Attachment'].apply(lambda x: 'D' in x and 'T' in x and (x.count('D') > 0 and x.count('T') > 0))
